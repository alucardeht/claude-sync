...
class SetupWizard {
  async run() {
    ...
    if (config.exists()) {
      ...
      try {
        previousWorkspaces = config.listWorkspaces();
        if (previousWorkspaces.length > 0) {
          console.log(chalk.blue(`\n  Preserving ${previousWorkspaces.length} workspace registration(s)...`));
          previousWorkspaces.forEach(ws => {
      ...
      for (const ws of previousWorkspaces) {
        try {
          if (fs.existsSync(ws.path)) {
            config.addWorkspace(ws.path);
            restoredCount++;
          } else {
            console.log(chalk.yellow(`  Workspace no longer exists, skipping: ${ws.path}`));
          }
        } catch (err) {
          if (!err.message.includes('already registered')) {
            console.log(chalk.yellow(`  Could not restore workspace: ${ws.path} (${err.message})`));
          }
        }
      }

      if (restoredCount > 0) {
        console.log(chalk.green(`  Restored ${restoredCount} workspace registration(s)`));
      }
    }

    await this.addInitialWorkspaces(previousWorkspaces.length > 0);
  ...
  async addInitialWorkspaces(hasRestoredWorkspaces = false) {
    console.log(chalk.blue.bold('\n📁 Step 4: Add Workspaces (Optional)\n'));

    if (hasRestoredWorkspaces) {
      const existing = config.listWorkspaces();
      console.log(chalk.gray(`Previously registered workspaces have been restored (${existing.length} total).`));
    }
    ...
    const { addNow } = await inquirer.prompt([
      {
        type: 'confirm',
        name: 'addNow',
        message: hasRestoredWorkspaces
          ? 'Do you want to add MORE workspaces?'
          : 'Do you want to add workspaces now?',
        default: !hasRestoredWorkspaces,
      },
    ]);
    ...
    while (continueAdding) {
      ...
      try {
        ...
        const globalPath = path.join(resolvedPath, syncRules.globalFile);
        const projectPath = path.join(resolvedPath, syncRules.projectFile);
        ...
        const syncResult = await syncer.syncWorkspace(resolvedPath);
        if (syncResult && syncResult.success === false) {
          console.log(chalk.yellow(`  Warning: Could not generate CLAUDE.md (sync returned failure)`));
        }

        console.log(chalk.green(`✓ Added workspace: ${added}`));
      } catch (error) {
        if (error.message && error.message.includes('already registered')) {
          console.log(chalk.yellow(`~ Workspace already registered: ${workspacePath}`));
        } else {
