...
class Syncer {
    ...
    if (fs.existsSync(globalFile)) {
      try {
        const gitResult = await git.syncFile(
          globalFile,
          `Update CLAUDE.md from ${workspaces[0].name} at ${new Date().toISOString()}`
        );
        results.push({
          workspace: 'GitHub',
          ...gitResult,
  ...
  async syncToGitHub(sourcePath) {
    ...
    const workspaceName = path.basename(path.dirname(sourcePath));

    try {
      const result = await git.syncFile(
        sourcePath,
        `Update CLAUDE.md from ${workspaceName} at ${new Date().toISOString()}`
      );
