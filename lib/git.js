...
class GitManager {
      ...
      if (error.message.includes('already exists') || error.message.includes('not empty')) {
        try {
          ...
          if (!fs.existsSync(readmePath)) {
            fs.writeFileSync(
              readmePath,
              '# Claude CLAUDE.md Global Sync\n\nThis repository contains the global CLAUDE.md file synchronized across all projects.\n',
              'utf8'
            );
          }
  ...
  async syncFile(filePath, commitMessage = 'Update CLAUDE.md') {
    if (!this.git) {
      this.git = simpleGit(this.repoPath);
    }
