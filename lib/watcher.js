  ...
  async setupWatchers() {
    ...
    for (const workspace of allWorkspaces) {
      if (fs.existsSync(workspace.path)) {
        ...
        try {
          config.removeWorkspace(workspace.path);
          console.log(chalk.yellow(`⚠ Auto-removed missing workspace: ${workspace.name} (${workspace.path})`));
        } catch (err) {
          console.log(chalk.red(`✗ Could not remove missing workspace: ${workspace.path}`));
        }
      }
    }

    if (removedWorkspaces.length > 0) {
      console.log(chalk.gray(`Removed ${removedWorkspaces.length} missing workspace(s)\n`));
    }

    if (validWorkspaces.length === 0) {
      throw new Error('No valid workspaces found. Use "claude-sync add <path>" to add workspaces.');
    }

    const workspaces = validWorkspaces;
